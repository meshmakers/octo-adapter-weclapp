...
public class WeClappTrialWriteSmokeTests(ITestOutputHelper output)
{
        ...
        string fileName, string content)
    {
        ...
        A.CallTo(() => nodeContext.Info(A<string>._, A<object[]>._)).Invokes(call =>
            output.WriteLine($"  node: {NodeLogCapture.Format(call)}"));
    ...
    public async Task ArWrite_LiveDryRun_ValidatesFullPutRoundTripAgainstRealShipment()
    {
        ...
        // Synthetic AR for that real order (golden field counts; carrier 9 is a meaningless
        // legacy placeholder that maps to nothing — the raw token still triggers one
        // carrier-list GET, and the trial has no shippingCarrier entities anyway).
        var arContent =
